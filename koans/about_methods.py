...
class AboutMethods(Koan):
    ...
    def test_calling_with_default_values(self):
        #default value is passed in since b was not called in the method
        self.assertEqual([1,'default_value'], self.method_with_defaults(1))
        self.assertEqual([1, 2], self.method_with_defaults(1, 2))
        # 2 was called so the default value did not take place

    # ------------------------------------------------------------------

    def method_with_var_args(self, *args):
        return args
        #args collect the positional arguments that are not defined and store them in a tuple
    ...
    def test_that_old_methods_are_hidden_by_redefinitions(self):
        #method is redefined to return 42
    ...
    def test_that_overlapped_method_is_still_there(self):
        ...
        #link_to_overlapped stored the original method that returns 10
    ...
    #similar to identifying javascript classes
    def test_attributes_with_double_underscore_prefixes_are_subject_to_name_mangling(self):
        rover = self.Dog()
        with self.assertRaises(AttributeError): password = rover.__password()#Dog object has no attribute
